...
def test_echappement_json():
    ...
    print(f"\n📝 PROMPT CONCATÉNÉ ({len(prompt_final)} chars):")
    print("-" * 30)
    print(prompt_final[:200] + "..." if len(prompt_final) > 200 else prompt_final)
    
    # Test de validation JSON
    is_valid, error_msg = manager.validate_json_for_template(prompt_final)
    ...
    # Test de construction JSON complète
    try:
        test_payload = f'{{"contents":[{{"parts":[{{"text":"{prompt_final}"}}]}}]}}'
